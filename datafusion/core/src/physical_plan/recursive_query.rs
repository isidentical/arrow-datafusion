...
/// a dynamic part (the recuursive term). The execution will start from
...
/// part will be continously executed.
///
...
/// an infinite recurison, so it is up to the planner to ensure that
/// it won't happen.
#[derive(Debug)]
    ...
    fn poll_next_iteration(
        ...
        for batch in self.buffer.drain(..) {
            match sender.try_send(Ok(batch)) {
                Ok(_) => {}
                Err(e) => {
                    return Poll::Ready(Some(Err(ArrowError::from_external_error(
                        Box::new(e),
                    ))))
                }
            }
